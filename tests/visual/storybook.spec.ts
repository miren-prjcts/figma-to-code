...
// Storybook visual-regression harness (DSV2-006).
//
...
// when a future ticket (e.g. a Phase 1 component) adds stories.
//
  ...
  for (const story of stories) {
    for (const theme of THEMES) {
      test(`${story.title} / ${story.name} [${theme}]`, async ({ page }) => {
        ...
        // first screenshot. See docs/VISUAL_REGRESSION.md's open risks.
        await page.evaluate(() => document.fonts.ready);

        // Full-page (not a component locator): Modal renders via a React portal to
        // document.body (packages/ui/src/components/modal.tsx), outside any single story-root
        // element, so a locator-scoped screenshot would silently miss it.
